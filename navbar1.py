...
color = {"nero": "#252726", "teal": "#159897", "darkorange": "#FE6201"}
...
def switch():
    ...
    if btnState is True:
        ...
        homeLabel.config(bg=color["teal"])
        topFrame.config(bg=color["teal"])
        root.config(bg="gray17")
...
# top Navigation bar:
topFrame = tk.Frame(root, bg=color["teal"])
topFrame.pack(side="top", fill=tk.X)

# Header label text:
homeLabel = tk.Label(topFrame, text="PE", font="Bahnschrift 15",
                     bg=color["teal"], fg="gray17", height=2, padx=20)
homeLabel.pack(side="right")

# Main label text:
brandLabel = tk.Label(root, text="Python \nNavbar",
                      font="System 30", bg="gray17", fg="teal")
brandLabel.place(x=100, y=250)

# Navbar button:
navbarBtn = tk.Button(topFrame, image=navIcon,
                      bg=color["teal"], activebackground=color["teal"], bd=0, padx=20, command=switch)
...
tk.Label(navRoot, font="Bahnschrift 15",
         bg=color["teal"], fg="black", height=2, width=300, padx=20).place(x=0, y=0)
...
for i in range(5):
    tk.Button(navRoot, text=options[i], font="BahnschriftLight 15", bg="gray17", fg=color["teal"],
              activebackground="gray17", activeforeground="green", bd=0).place(x=25, y=y)
    y += 40

# Navbar Close Button:
closeBtn = tk.Button(navRoot, image=closeIcon,
                     bg=color["teal"], activebackground=color["teal"], bd=0, command=switch)
